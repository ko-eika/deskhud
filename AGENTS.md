...
- 第一版运行态与设置页统一由 `deskhud-egui` 承载；宠物通过平台无关场景契约绘制。原生平台后端仅在宠物引擎协议稳定后再评估，禁止恢复 eframe/deferred 双路径。
...
- 运行态 HUD 必须**每屏一个合成窗、同层绘制**；启用条件是总开关 ∧ 插件 ∧ 实例 ∧（若有）组，且合成窗不要每帧设置 `WindowLevel`。
...
| 领域 | 选择 | 说明 |
|------|------|------|
| UI | `deskhud-egui` 首版运行态与设置页 | 先用 egui 验证宠物引擎；原生平台窗口后置评估；无 eframe、无托盘 |

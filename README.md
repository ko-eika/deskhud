...
当前版本：`0.6.4`。Windows 宠物覆盖层已可用，macOS 已接入独立平台窗口与 `NSScreen.visibleFrame` 安全区；HUD 的真实帧数据与原生合成仍是当前主要缺口。macOS 菜单文字错位/多窗重绘冻结已完成代码修复，待 macOS 实机验收，见 [`docs/issues/macos-menu-text-offset.md`](./docs/issues/macos-menu-text-offset.md)。

...
<p align="center">
    ...
    <img src="https://img.shields.io/badge/version-0.9.8-green.svg" alt="version">
...
DeskHud is an extensible **desktop pet engine**: switch **pet packs** (look + behavior), and toggle **HUD plugins** and their contributions. The UI is built with **egui + winit / egui_glow**, with multi-language support and local community pack loading (a store comes later). Current release: `0.9.8`, with a cached built-in system monitor for real system, DeskHud, and selected-application CPU/memory metrics, plus cross-plugin HUD groups and per-instance visual editing.
...
| Config | `serde` + TOML prefs / manifest; pack i18n uses gettext PO/MO |
...
git tag -a v0.9.8 -m "DeskHud 0.9.8"
...
The development build embeds the single Inter 4.1 collection from [`assets/fonts/Inter.ttc`](./assets/fonts/Inter.ttc).

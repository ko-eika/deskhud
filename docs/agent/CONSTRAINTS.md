...
- 运行态 HUD：**每屏一个合成窗、同层绘制**；启用 = 总开关 ∧ 插件 ∧ 实例 ∧（若有）组；普通模式使用 `[prefs]` 的 `hud.size`/`hud.position` 窗口预设，布局模式临时覆盖活动区；`HudSlotLayout` 的 `x/y` 使用相对 HUD 窗口的物理像素，宽高仍为缩放因子。
- HUD 定义、实例、组分离：持久化布局与成员引用以宿主生成的稳定实例 ID 为主键，标题不得作为身份；一个实例最多属于一个组，组不得嵌套。插件或 contribution 暂缺时保留实例与组关系，单项损坏不得阻止其它 HUD 配置加载。
- HUD 组内排版只使用 engine 的中性逻辑尺寸、矩形与裁剪结果，支持自由、横向、纵向和网格布局；组是一个使用自身屏幕布局的虚拟 HUD 槽，成员几何与实例样式独立持久化，成员不得同时使用独立屏幕坐标，渲染器不得为组或成员新增窗口。组的创建、成员拖放与调整统一位于 HUD 布局模式，不在插件设置页另设组管理入口。

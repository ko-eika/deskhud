...
| 2026-09-01 | 产品与内置包 PATCH 升至 `0.9.1`，engine 兼容族保持 `0.9`，Guest `api_version` 保持 4 | 模块化设置页绘制与通用配置组件；补齐常规、关于、宠物和 HUD 设置的中英文文案 |
...
| 2026-09-02 | 下一条主动路线确定为 HUD 组基础能力 → 系统与应用性能插件 → 多实例便签插件；HUD 定义、实例与分组分离 | 先建立跨插件组合与稳定实例身份，再用真实系统数据和多个便签验证同一套配置、布局及渲染模型；演示插件删除不纳入该计划 |
| 2026-09-02 | 完成 HUD 组阶段 1.1 契约与持久化：来源、实例 ID、帧上下文和组内布局进入 engine，中性实例/组模型进入 UI prefs | 旧 contribution 幂等映射为确定性默认实例；缺失来源原样保留，损坏项独立恢复，重复成员保留首次归属；未修改 Guest/WIT，`api_version = 4` |
| 2026-09-03 | 产品与内置包 PATCH 升至 `0.9.4`，完成 HUD 组阶段 1.1 并将实例/组配置改为可读 TOML 数组表 | 旧静态条目一次性迁移为稳定默认实例；保留缺失来源并逐项恢复损坏配置；engine 兼容族保持 `0.9`，Guest `api_version` 保持 4 |
| 2026-09-03 | 完成 HUD 组阶段 1.2 组合与渲染，运行态改为实例解析和虚拟组槽 | engine 输出中性组内矩形与裁剪；支持横向、纵向、网格、间距、内边距和三档对齐；启用门控加入实例及组层，仍保持每屏一个合成窗；Guest/WIT ABI 未修改 |
| 2026-09-03 | 产品与内置包 PATCH 升至 `0.9.6`，完成 HUD 组阶段 1.3 布局模式分组与编辑 | 布局模式右键创建和管理虚拟组，支持成员拖放、自由/自动排布及实例级样式；HUD 绘制按职责拆分，engine 族保持 `0.9`，Guest/WIT ABI 保持 4 |
| 2026-09-03 | 产品与内置包 PATCH 升至 `0.9.5`，完成 HUD 组阶段 1.2 组合与渲染 | 跨插件实例可组合为统一虚拟槽并进行中性变换与裁剪；engine 兼容族保持 `0.9`，Guest `api_version` 保持 4 |
| 2026-09-05 | 产品与内置包 PATCH 升至 `0.9.7`，统一 HUD 合成窗口几何与组布局编辑模型 | 普通模式保存窗口尺寸/位置；HUD 槽位改用窗口相对物理像素坐标，组默认自由布局并修正实时调整与尺寸约束；engine 兼容族保持 `0.9`，Guest/WIT ABI 保持 4 |
| 2026-09-05 | 产品与内置包 PATCH 升至 `0.9.8`，完成系统与应用性能 HUD 插件 | 官方原生插件通过 `sysinfo` 后台按需缓存指标；系统状态、当前应用进程状态与指定应用进程状态统一为同尺寸、CPU/内存同行（内存百分比保留在同一行）、趋势图、双色占用条和底部详情的三条独立 HUD，旧缩放值一次性归一，当前应用卡在半秒窗口上统计并平滑显示实际 HUD 渲染 FPS；文本阴影默认色从整帧最亮正文推导，避免 Label 首项颜色导致 system 与 demo 阴影观感不一致；设置页按条目提供独立开关，应用监控通过插件动态提供的可搜索进程下拉框选择；新增中性锚点文本、进度条、历史曲线和实例配置声明，宿主按宽高轴分别缩放逻辑坐标，engine 兼容族保持 `0.9`，Guest/WIT ABI 保持 4 |
| 2026-09-05 | 产品与内置包 PATCH 升至 `0.9.9`，完善 HUD 布局编辑会话与导航面板 | 新增信息树/活动树、对齐辅助线与吸附、位置/尺寸重置及单项关闭；布局支持应用或放弃，修正 Escape、窗口关闭、异步移动后的提交和恢复；engine 兼容族保持 `0.9`，Guest/WIT ABI 保持 4 |

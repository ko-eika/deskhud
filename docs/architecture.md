...
- 启用门控为总开关 ∧ 插件 ∧ 实例 ∧（若有）组；插件关 → 其下全部不显示
  - 可继续加同前缀自定义键，便于扩展
- 布局编辑：关闭设置后打开 Windows 原生 D3D11 + DirectComposition 全屏编辑视口；按任务栏工作区绘制半透明安全边界。布局模式通过右键菜单创建和管理虚拟组，支持 HUD 拖入/移出、自由或自动组内排布，并分别调整组与组内实例；组宽高使用实际逻辑像素（不设上限），相对 HUD 窗口的屏幕位置写入 `HudSlotLayout`，成员几何和实例样式独立持久化，运行态 HUD 仍使用每屏一个原生合成窗。

...
- prefs：
  ...
  - 插件关 → 其下全部不显示
  - 可继续加同前缀自定义键，便于扩展
- 布局编辑：关闭设置/菜单后打开 Windows 原生 D3D11 + DirectComposition 全屏编辑视口；按任务栏工作区绘制半透明安全边界，选中 HUD 使用半透明实线边框，右下角显示三角缩放角标。当前先支持主屏，布局修改写入 `HudSlotLayout`；运行态 HUD 每屏合成仍在迁移中。

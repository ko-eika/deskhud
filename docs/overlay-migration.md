...
3. **GPU 可视探针（单显示器通过）**：`DESKHUD_GPU_PROBE=1` 只确认硬件 D3D11 与 DirectComposition 能力；`DESKHUD_GPU_OVERLAY_PROBE=1` 在独立原生小窗中以 Direct2D 持续绘制透明呼吸圆形，再由 DirectComposition 合成。透明、置顶与呼吸流畅度已人工验收。二者都不影响 GDI 探针或默认路径。下一步迁移真实宠物绘制与输入。
4. **场景与输入接线**：宠物及 HUD 产出中性场景/命中区域；现有 `PetPaint` 的身体、眼白、瞳孔、呼吸缩放和身体命中区已可转换为 `OverlayScene`，对话气泡文字和实时宠物运行时仍待接线。Windows 后端之后将系统输入路由为现有 `PetEvent` 与 HUD 事件，完成后才接管正式运行态。
5. **单显示器正式版**：默认选择一台显示器，按其工作区布局；设置页展示能力与降级原因。旧 eframe 路径保留为可选回退。

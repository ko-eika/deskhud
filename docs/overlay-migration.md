...
Windows 默认路径使用原生 GPU 宠物窗与直接托管的 egui 控制窗；旧 eframe 运行路径已经移除，避免维护两套窗口生命周期。非 Windows 当前只保留可编译的控制宿主骨架，透明宠物/HUD 必须由对应平台后端接管后才算可用。HUD 原生合成尚未接管，因此 `0.5.1` 仍是迁移版本，而不是全部后端完成。

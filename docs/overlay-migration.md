...
Windows 默认路径使用原生 GPU 宠物、菜单、气泡和 HUD 窗口；设置页暂由 `winit + egui_glow` 托管。旧的双路径运行方案已经移除，避免维护两套窗口生命周期。macOS/Linux 的正式原生窗口后端按里程碑推进，fallback 仅表示明确降级，不代表正式平台适配完成。当前产品版本为 `0.6.17`。
